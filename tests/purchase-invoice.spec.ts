...
test('save functionality saves the invoice data', async ({ page }) => {
  ...
  try {
    initialInvoiceCount = await page.locator('.invoice-row').count();
  } catch (e) {
  ...
  // Generate unique invoice number to avoid conflicts with previous test runs
  ...
  const newInvoiceCount = await page.locator('.invoice-row').count();
  ...
  await expect(invoiceRows.filter({ hasText: uniqueDescription })).toBeVisible();
  await expect(invoiceRows.filter({ hasText: uniqueInvoiceNumber })).toBeVisible();
});
